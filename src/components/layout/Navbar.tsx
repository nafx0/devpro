...
function MobileNavItem({
    ...
    onNavClick: (e: React.MouseEvent, href: string, anchorId: string) => void;
}) {
        ...
        <div className="mobile-nav-item border-b border-deep-forest/5 last:border-0">
            <div className="flex items-center justify-between py-2">
                <Link
                    ...
                    onClick={(e) => onNavClick(e, href, anchorId)}
                    className={clsx(
...
export default function Navbar() {
    ...
    // Render rules:
    // - Remove nesting from Home (no dropdown / accordion)
    // - Remove Contact Us from navbar
    const navCategories = useMemo(() => {
        return navigation
            .filter((cat) => cat.label !== "Contact Us")
            .map((cat) => (cat.label === "Home" ? { ...cat, children: [] } : cat));
    }, []);
    ...
    useEffect(() => {
        ...
        const sections = navCategories.map((cat) => cat.anchorId);
    ...
    }, [isHome, pathname, navCategories]);

    // Conditional Click Logic: Anchor vs Route
    const handleNavClick = useCallback((e: React.MouseEvent, href: string, anchorId: string) => {
    ...
    // Mobile Menu: no slow open animations (snappy open/close)
            ...
            <nav ref={navRef} className={clsx(
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between">
                        <Link href="/" className="relative z-50 h-20 flex items-center" onClick={(e) => handleNavClick(e, "/", "home")} aria-label="DeVPro Home">
                            <Image src={logoImg} alt="DeVPro" width={210} height={64} className="h-12 w-auto object-contain" priority />
                        </Link>
                                                            ...
                                                            return (
                                                                <div key={child.slug} className="relative group/sub">
                                                                    <Link
                                                                        ...
                                                                        onClick={(e) => handleNavClick(e, `${cat.href}/${child.slug}`, "")}
                                                                                ...
                                                                                <div className="flex flex-col gap-1">
                                                                                    {child.children!.map((subChild) => (
                                                                                        <Link
                                                                                            ...
                                                                                            onClick={(e) => handleNavClick(e, `${cat.href}/${child.slug}/${subChild.slug}`, "")}
